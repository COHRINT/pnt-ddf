...
class Agent_Wrapper:
    def __init__(self):
        ...
        agent = Agent(env, self.agent_name)
...
class AgentWrapper(object):
    ...
    def __init__(self, config_path=None):
        ...
        self.ordered_ids = sorted(
            list(set(self.ordered_ids))
        ...
        start_state = np.array(
            (start_state["x"], 0, start_state["y"], 0, start_state["z"], 0)
        )
        ...
        # initialize ros node
        ROS_LOG_LEVEL = eval(
            "rospy." + rospy.get_param("log_level")
        )  # self.get_log_level(rospy.get_param('log_level'))
        ...
        self.agent = self.init_agent(
            start_state, dynamics_fxn, dynamics_fxn_params, sensors
        )
        ...
        rospy.Subscriber(
            "agent_meas_incoming", AgentMeasurement, self.queue_received_measurement
        )
        # rospy.Subscriber('agent_state', AgentState, self.queue_received_state)

        # create publishers to comms module
        self.comms_meas_pub = rospy.Publisher(
            "agent_meas_outgoing", AgentMeasurement, queue_size=10
        )
        self.comms_state_pub = rospy.Publisher(
            "agent_state_outgoing", AgentState, queue_size=10
        )

        # create publisher of local esimate
        self.local_est_pub = rospy.Publisher(
            "local_estimate", AgentState, queue_size=10
        )
        # create ownship-only publisher
        self.local_est_ownship_pub = rospy.Publisher(
            "local_estimate_ownship", AgentState, queue_size=10
        )

        # create message statistics publisher and timer
        self.msg_stats_pub = rospy.Publisher("msg_stats", MsgStats, queue_size=10)
        self.msg_stats_timer = rospy.Timer(
            rospy.Duration(rospy.get_param("msg_stats_rate")), self.publish_msg_stats
        )
    ...
    def process_local_measurement_queue(self):
        ...
        local_measurements = [
            self.local_measurement_queue.get() for x in range(num_measurements)
        ]

        # throw away measurements older than one update cycle
        for msg in local_measurements:
            if rospy.Time.now() - msg.header.stamp > rospy.Duration(
                secs=self.update_rate
            ):
                ...
                del msg
                rospy.logwarn(
                    "[ET-DDF Agent {}]: throwing away local {} message -- {} sec old".format(
                        self.agent_id, msg_type, how_old
                    )
                )
    ...
    def process_received_measurement_queue(self):
        ...
        received_measurements = [
            self.received_measurement_queue.get() for x in range(num_measurements)
        ]
        ...
        for msg in received_measurements:
            if rospy.Time.now() - msg.header.stamp > rospy.Duration(
                ...
                del msg
                rospy.logwarn(
                    "[ET-DDF Agent {}]: throwing away {} message from Agent {} -- {} sec old".format(
                        self.agent_id, msg_type, src_agent, how_old
                    )
                )
    ...
    def local_ci(self):
        ...
        if np.trace(self.agent.local_filter.P) > self.agent.tau:
            ...
            for i, conn in enumerate(
                self.connections[self.ordered_connections.index(self.agent_id)]
            ):
                # request state of connection and queue
                rospy.logdebug(
                    "[ET-DDF Agent {}]: waiting for CI update service from agent_{} to become available".format(
                        self.agent_id, conn
                    )
                )
                ...
                request_msg = python2ros_state(
                    self.agent.gen_ci_message(conn, self.neighbor_connections[conn])
                )
                ...
                try:
                    ...
                    rospy.logdebug(
                        "[ET-DDF Agent {}]: CI update response received".format(
                            self.agent_id
                        )
                    )
                    ...
                    rospy.logerr(
                        "[ET-DDF Agent {}]: CI update service request failed: {}".format(
                            self.agent_id, e
                        )
                    )
    ...
    def process_ci_queue(self):
        ...
        """
        rospy.logdebug(
            "[ET-DDF Agent {}]: Emptying CI message queue...".format(self.agent_id)
        )
        ...
        for msg in ci_messages:
            if rospy.Time.now() - msg.header.stamp > rospy.Duration(
                ...
                del msg
                rospy.logwarn(
                    "[ET-DDF Agent {}]: throwing away CI message from Agent {} -- {} sec old".format(
                        self.agent_id, src_agent, how_old
                    )
                )

        rospy.logdebug(
            "[ET-DDF Agent {}]: Grabbed {} message(s) from CI message queue.".format(
                self.agent_id, num_messages
            )
        )
    ...
    def ci_service_handler(self, req):
        ...
        """
        rospy.logdebug(
            "[ET-DDF Agent {}]: CI update request received".format(self.agent_id)
        )
        # queue recevied request message
        self.ci_queue.put(deepcopy(req.request_state))

        # generate ci message for response
        res_msg = self.agent.gen_ci_message(
            req.request_state.src, self.neighbor_connections[req.request_state.src]
        )
    ...
    def update(self):
        ...
        rospy.logdebug(
            "[ET-DDF Agent {}] local_measurements: ".format(self.agent_id)
            + str(local_measurements)
        )
        ...
        local_measurements_generic = gen_measurement_msg(
            self.agent_id, local_measurements
        )
        local_measurements_python = ros2python_measurement(local_measurements_generic)

        # pass measurements to wrapped agent
        threshold_measurements = self.agent.process_local_measurements(
            self.recent_control_input, local_measurements_python
        )
        ...
        for m in ci_messages_ros:
            ...
            assert (type(m.src_connections) is list) or (
                (type(m.src_connections) is int) or (type(m.src_connections) is tuple)
            )
    ...
    def init_agent(
        self, start_state, dynamics_fxn="lin_ncv", dynamics_fxn_params={}, sensors={}
    ):
        """
        ...
        for j in range(0, len(self.connections[agent_id_index])):
            for k in range(
                ...
                len(
                    self.connections[
                        self.ordered_connections.index(
                            self.connections[agent_id_index][j]
                        )
                    ]
                ),
            ):
                ...
                if (
                    ...
                    in neighbor_conn_ids
                ):
                    neighbor_conn_ids += self.connections[
                        self.ordered_connections.index(
                            self.connections[agent_id_index][j]
                        )
                    ]
        ...
        connections_new = list(
            set(sorted(neighbor_conn_ids + self.connections[agent_id_index]))
        )
        ...
        for i in self.connections[agent_id_index]:
            if i is not self.agent_id:
                ...
                neighbor_agent_id_index = self.ordered_connections.index(
                    neighbor_agent_id
                )
                ...
                for j in range(0, len(self.connections[neighbor_agent_id_index])):
                    for k in range(
                        ...
                        len(
                            self.connections[
                                self.ordered_connections.index(
                                    self.connections[neighbor_agent_id_index][j]
                                )
                            ]
                        ),
                    ):
                ...
                # divide out direct measurement connections and all connections
                neighbor_connections_new = list(
                    set(
                        sorted(
                            neighbor_conn_ids
                            + deepcopy(self.connections[neighbor_agent_id_index])
                        )
                    )
                )
                meas_connections_new = deepcopy(
                    self.connections[neighbor_agent_id_index]
                )
        ...
        F, G, Q = globals()[dynamics_fxn](
            self.update_rate, est_state_length, **dynamics_fxn_params
        )
        ...
        for j in range(0, len(meas_connections)):
            ...
            unique_states = set(
                meas_connections
                + self.connections[
                    self.ordered_connections.index(self.connections[agent_id_index][j])
                ]
            )
            ...
            # generate dynamics
            F_comm, G_comm, Q_comm = globals()[dynamics_fxn](
                self.update_rate, len(comm_ids), **dynamics_fxn_params
            )
            ...
            comm_filter = ETKF(
                ...
                np.array(R_abs),
                np.array(R_rel),
                x0_comm.reshape((F_comm.shape[0], 1)),
                P0_comm,
                self.delta,
                agent_id,
                comm_ids,
        ...
        agent = Agent(
            ...
            start_state,
            0,
            len(x0) * self.tau_goal,
            len(x0) * self.tau,
            self.use_adaptive_tau,
            self.quantization_flag,
            self.diagonalization_flag,
            epsilon1=self.epsilon_1,
            epsilon2=self.epsilon_2,
        )

        rospy.loginfo(
            "[ET-DDF Agent {}]: Agent initialized w/ delta -- {} \t tau_goal -- {} \t tau -- {}".format(
                agent_id, self.delta, len(x0) * self.tau_goal, len(x0) * self.tau
            )
        )

        rospy.logwarn(
            "[ET-DDF Agent {}]: connections -- {}, meas connections -- {}".format(
                agent_id, connections_new, meas_connections
            )
        )
